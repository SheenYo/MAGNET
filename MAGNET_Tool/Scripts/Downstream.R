...
options(download.file.method="libcurl",useHTTPS=FALSE,BioC_mirror = "http://bioconductor.org",repos = c(CRAN = "http://cran.r-project.org", CRANextra = "http://cran.uk.r-project.org"))

	if (!requireNamespace("BiocManager", quietly = TRUE))
	    install.packages("BiocManager")
	    pkgs <- rownames(installed.packages())
	    BiocManager::install(pkgs, type = "source", checkBuilt = TRUE,ask=FALSE,lib.loc="./tmpRlib",lib="./tmpRlib")

package<-c("codetools","igraph")
	if(length(setdiff(package, rownames(installed.packages(lib.loc="./tmpRlib")))) > 0)	{
	install.packages(setdiff(package, rownames(installed.packages(lib.loc="./tmpRlib"))),lib="./tmpRlib",ask=FALSE)  
												}
require("BiocManager",lib.loc="./tmpRlib")


##update.packages(checkBuilt = TRUE, ask = FALSE,lib.loc="./tmpRlib")
...
packages2_2<-c("gtools","gplots","GO.db","Hmisc","illuminaio","impute","lattice","limma","Matrix","mime","mvtnorm","nlme","openssl",
"org.Hs.eg.db","plyr","preprocessCore","rmarkdown","rpart","rrcov","RSQLite","scales","shiny","survival","tables","WGCNA","XML")

	if (length(setdiff(packages2_2, rownames(installed.packages(lib.loc="./tmpRlib")))) > 0) {
	.libPaths("./tmpRlib")
	BiocManager::install(setdiff(packages2_2, rownames(installed.packages(lib.loc="./tmpRlib"))),site_repository="http://bioconductor.org/packages/3.9/bioc",lib="./tmpRlib",ask=FALSE)
	} 
...
if(!length(args)==5){
  ...
  GOElitefolder="./Output/goelite_output"
  Outputfolder="OutputDownstream"
  pheno="Phenotype"
  MAGNETHome="../MAGNET"
  Genelist=c("Genelist")
} else {
  GOElitefolder=args[1]
  Outputfolder=args[2]
  pheno=args[3]
  MAGNETHome=args[4]
  Genelist=args[5]
...
Home=getwd()
#Generating Biological Plots

##GO plots
setwd(GOElitefolder)

Files=list.files("GO-Elite_results/CompleteResults/ORA_pruned/")

ZscoresGO=Files[grep("GO_z-score", Files)]

GOoutput = read.table(paste0("GO-Elite_results/CompleteResults/ORA_pruned/",ZscoresGO),
...
if(identical(GOoutput$Ontology.ID,logical(0))==FALSE) {
  ...
  pdf(paste0(pheno,"GO_ElitePlot.pdf"),height=4,width=5);
  par(oma=c(3,6,3,6));
  par(mgp=c(1, 0, 0))
...
topKEGG_Pheno$logP<--log10(topKEGG_Pheno$P.DE)
write.table(topKEGG_Pheno,paste0("topKEGG_",pheno,".txt"),sep="\t",row.names=FALSE,quote=FALSE)


setwd(Home)
setwd(Outputfolder)
pdf(paste0(pheno,"KEGG_ElitePlot.pdf"),height=4,width=5);
par(oma=c(3,6,3,6));
par(mgp=c(1, 0, 0))
bp = barplot(topKEGG_Pheno$logP[10:1],
             main=paste("GO Ontology", pheno),
             horiz=TRUE,
             yaxt='n',col="#fec44f",
...
axis(2,at=bp,labels=topKEGG_Pheno$Pathway[10:1],
     tick=FALSE,las=2,cex.axis=0.3);

abline(v=1.3,col="red",lwd=2,lty=1);
...
magmagenes_significant<-read.table(Genelist,sep="\t")
